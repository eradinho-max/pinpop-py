...
check('Frontend no contiene valores secretos de ADMIN_PASSWORD', !/ADMIN_PASSWORD\s*=|process\.env\.ADMIN_PASSWORD/.test(publicText));
...
check('Login exige contraseña + TOTP después del primer acceso', /password, totp/.test(server) && /verifyTotp/.test(server));
check('2FA se configura dentro del panel, sin variable ADMIN_TOTP_SECRET', /api\/auth\/setup\/start/.test(server) && /api\/auth\/setup\/confirm/.test(server) && !/ADMIN_TOTP_SECRET/.test(server));
check('QR 2FA se genera en el servidor', /QRCode\.toDataURL/.test(server) && pkg.dependencies.qrcode === '1.5.4');
...
check('No hay token admin en sessionStorage/localStorage', !/pinpop_admin_jwt|sessionStorage/.test(app));
check('Netlify Blobs es la persistencia de datos', /@netlify\/blobs/.test(db) && /setJSON/.test(db));
check('Escrituras de estado usan ETag', /onlyIfMatch/.test(db));
check('Imágenes usan Netlify Blobs', /@netlify\/blobs/.test(storage) && /pinpop-media/.test(storage));
check('Upload valida magic bytes', /detectImageType/.test(storage) && /assertValidImage/.test(storage));
check('CSP scripts self-only', /scriptSrc:\s*\["'self'"\]/.test(server) && /script-src 'self'/.test(headers));
